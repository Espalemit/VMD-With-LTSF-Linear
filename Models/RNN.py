...
class MVSimpRNN(nn.Module):
  device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
  def __init__(self, feat_lenn, inp_size, hid_size, num_stack_layer):
    super().__init__()
    self.hid_size = hid_size
    self.num_stack_layer = num_stack_layer
    self.rnn = nn.RNN(inp_size, hid_size, num_stack_layer, batch_first=True)
    self.Linear = nn.Linear(hid_size, feat_lenn)
